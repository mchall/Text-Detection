...
PolyGroundTruthSet LinkCandidate::run(vector<Candidate>& candidateStore)
{
...
void LinkCandidate::finalresult()
{
	...
	for (int i = 1; i < index; i++)
	{
		...
		{
			PointV.push_back(Point(candidateStore[store[i][j]].min_j, candidateStore[store[i][j]].min_i));
			PointV.push_back(Point(candidateStore[store[i][j]].max_j, candidateStore[store[i][j]].max_i));
		}

		Rect textline = boundingRect(PointV);
