...
{
	CharacterClassifier = Algorithm::load<RTrees>("./resource/CharacterClassifier");

	oriBgrImage_8UC3 = Image;
	cvtColor(oriBgrImage_8UC3, gray_source_image, CV_BGR2GRAY);
	featureExtract();
	swtprocess();
	Filter();
...
void GetCandidate::featureExtract()
{
	...
	vector<Rect> bboxes;
	MSER::create()->detectRegions(this->gray_source_image, this->strVectorStore, bboxes);
}
...
void GetCandidate::ExtractCCfeatures()
{
	...
	for (int i = 0; i < Num; i++)
	{
		...
		for (int j = 1; j < cd.pointsNum; j++)
		{
			...
			tempr += this->oriBgrImage_8UC3.at<Vec3b>(strVectorStore[i][j].y, strVectorStore[i][j].x)[0];
			tempg += this->oriBgrImage_8UC3.at<Vec3b>(strVectorStore[i][j].y, strVectorStore[i][j].x)[1];
			tempb += this->oriBgrImage_8UC3.at<Vec3b>(strVectorStore[i][j].y, strVectorStore[i][j].x)[2];
		}
...
void GetCandidate::Filter()
{
	...
	for (int i = 0; i < ccStore.size(); i++)
	{
		...
		Mat rtVector = Mat(1, 14, CV_32FC1);
