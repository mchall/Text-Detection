...
PolyGroundTruthSet VericaficationTextLine::run(PolyGroundTruthSet& cgts, Mat& rgb)
{
	this->cgts = cgts;
	this->clearInnerBox();
...
void VericaficationTextLine::FilterBoxWithPCAandRandomForest(Mat& rgb)
{
	vector<PolyGroundTruth> newset;
	vector<PolyGroundTruth>& sets = this->cgts.sets;
	for (int i = 0; i < sets.size(); i++)
	{
		Mat rgb_copy = rgb; Mat cropped;
		double midx = sets[i].rr.x + sets[i].rr.width / 2;
		double midy = sets[i].rr.y + sets[i].rr.height / 2;
		getRectSubPix(rgb_copy, sets[i].rr.size(), Point2f(midx, midy), cropped, -1);
		resize(cropped, cropped, Size(200, 60));
