...
void Swt::initialize(Mat& image, vector<vector<Point2i>>& cc)
{
	image.copyTo(this->GrayImage);
	this->height = GrayImage.rows;
	this->width = GrayImage.cols;
	EdgeImage = Mat::zeros(this->height, this->width, CV_8UC1);
	StrokeImage = Mat::zeros(this->height, this->width, CV_32FC1);
	Edges(cc);
	StrokeImage = Mat::zeros(this->height, this->width, CV_32FC1);
	//imshow("Edges", this->EdgeImage);waitKey(10);
	gradient();
	td_findPairLine();
	inversegradient();
	td_findPairLine();
...
void Swt::gradient()
{
	max_x = Mat::zeros(this->height, this->width, CV_32FC1);
	max_y = Mat::zeros(this->height, this->width, CV_32FC1);
	float temp_y = 0;
	float temp_x = 0;
	float regular = 0;
...
void Swt::Edges(vector<vector<Point2i>>& cc)
{
	int ccsize = cc.size();
...
void Swt::displayStrokeWidth(vector<vector<Point2i>>& cc)
{
	Mat show = Mat::zeros(height, width, CV_8UC3);
	Vec3b  point = Vec3b(0, 0, 0);
	...
	for (int i = 0; i < ccsize; i++)
	{
		...
		for (int j = 0; j < ccisize; j++)
		{
			...
			show.at<Vec3b>(iindex, jindex) = point;
		}
	}
	imshow(" inner Stroke width", show);
	waitKey(10);
}

void Swt::getStrokeWidth(vector<Point2i>& component, Candidate& cad)
{
